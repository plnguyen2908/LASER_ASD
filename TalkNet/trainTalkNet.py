...
from dataLoader_talknet import train_loader, val_loader, val_loader_ASW, val_loader_talkies
...
def main():
    ...
    parser.add_argument('--savePath',     type=str, default="/nobackup/le/LoCoNet/talknet_landmark_lambda_0")
    ...
    parser.add_argument('--test',      dest='test', action='store_true', help='Only do testing shift/reverse by using pretrained model [pretrain_AVA.model]')
    parser.add_argument('--ASW',      dest='ASW', action='store_true')
    parser.add_argument('--talkies',      dest='talkies', action='store_true')
    ...
    loader = val_loader(trialFileName = args.evalTrialAVA, \
                        ...
                        landmark_path = os.path.join(args.trialPathAVA, 'landmark', 'val'),\
                        **vars(args))
    
    if args.swap:
        loader = val_loader(trialFileName = args.evalTrialAVA, \
        audioPath     = os.path.join(args.dataPathAVA , 'clips_audios_swap', 'val'), \
        visualPath    = os.path.join(args.visualPathAVA, args.evalDataType), \
        landmark_path = os.path.join(args.trialPathAVA, 'landmark', 'val'),\
        **vars(args))
    if args.test:
        loader = val_loader(trialFileName = args.shiftTrialAVA, \
                        audioPath     = os.path.join(args.dataPathAVA, f'clips_audios_shifted_{args.shift_factor}s' , 'test_shift'), \
                        visualPath    = os.path.join(args.visualPathAVA, args.evalDataType), \
                        landmark_path = os.path.join(args.trialPathAVA, 'landmark', 'val'),\
                        **vars(args))
        
    if args.ASW:
        loader = val_loader_ASW(trialFileName = args.evalTrialAVA, \
                        audioPath     = os.path.join(args.audioPathAVA , args.evalDataType), \
                        visualPath    = os.path.join(args.visualPathAVA, args.evalDataType), \
                        landmark_path = os.path.join(args.trialPathAVA, 'landmark', 'val'),\
                        **vars(args))
        if args.test:
            loader = val_loader_ASW(trialFileName = args.shiftTrialAVA, \
                        audioPath     = os.path.join(args.dataPathAVA, f'clips_audios_shifted_{args.shift_factor}s' , 'test_shift'), \
                        visualPath    = os.path.join(args.visualPathAVA, args.evalDataType), \
                        landmark_path = os.path.join(args.trialPathAVA, 'landmark', 'val'),\
                        **vars(args))
    ...
    if args.talkies:
        loader = val_loader_talkies(trialFileName = args.evalTrialAVA, \
                        audioPath     = os.path.join(args.audioPathAVA , args.evalDataType), \
                        visualPath    = os.path.join(args.visualPathAVA, args.evalDataType), \
                        landmark_path = os.path.join(args.trialPathAVA, 'landmark', 'val'),\
                        **vars(args))
        if args.test:
            loader = val_loader_talkies(trialFileName = args.shiftTrialAVA, \
                        audioPath     = os.path.join(args.dataPathAVA, f'clips_audios_shifted_{args.shift_factor}s' , 'test_shift'), \
                        visualPath    = os.path.join(args.visualPathAVA, args.evalDataType), \
                        landmark_path = os.path.join(args.trialPathAVA, 'landmark', 'val'),\
                        **vars(args))
    ...
    if args.evaluation == True:
        ...
        s = talkNet(n_channel=4, layer=1, **vars(args))
        s.loadParameters('/nobackup/le/LoCoNet/talknet_landmark/model/model_0024.model')
        s.loadParameters('/nobackup/le/LoCoNet/talknet_landmark_lambda_0/model/model_0025.model')
        print("Model %s loaded from previous state!"%('pretrain_AVA.model'))
        mAP = s.evaluate_network(loader = valLoader, useLandmark=True, **vars(args))
        print("mAP %2.2f%%"%(mAP))
        quit()

    if args.test:
        args.evalOrig = os.path.join(args.trialPathAVA, f'val_orig_shifted_{args.shift_factor}s.csv')
        args.evalCsvSave = os.path.join(args.savePath, "res_shifted_{}.csv".format(args.shift_factor))
        s = talkNet(n_channel=4, layer=1, **vars(args))
        s.loadParameters('/nobackup/le/LoCoNet/talknet_landmark/model/model_0024.model')
        # print("Model %s loaded from previous state!"%('pretrain_AVA.model'))
        mAP = s.evaluate(loader = valLoader, useLandmark=False, **vars(args))
        print("mAP %2.2f%%"%(mAP))
        quit()

    if args.swap:
        args.evalOrig = os.path.join(args.trialPathAVA, f'val_orig_modified.csv')
        args.evalCsvSave = os.path.join(args.savePath, "res_shifted_{}.csv".format(args.shift_factor))
        s = talkNet(n_channel=4, layer=1, **vars(args))
        s.loadParameters('/nobackup/le/LoCoNet/talknet_landmark/model/model_0024.model')
        # print("Model %s loaded from previous state!"%('pretrain_AVA.model'))
        mAP = s.evaluate(loader = valLoader, **vars(args))
        print("mAP %2.2f%%"%(mAP))
        quit()
    ...
    loader = train_loader(trialFileName = args.trainTrialAVA, \
                          ...
                          landmark_path = os.path.join(args.trialPathAVA, 'landmark', 'train'),\
                          **vars(args))
    ...
    if len(modelfiles) >= 1:
        ...
        epoch = 1
        s = talkNet(epoch = epoch, n_channel=4, layer=1, **vars(args))

    mAPs = []
    scoreFile = open(args.scoreSavePath, "a+")

    while(1):        
        loss, lr = s.train_network(epoch = epoch, loader = trainLoader, weight=0, **vars(args))
